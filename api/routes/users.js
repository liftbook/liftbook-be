...
router.get("/:username", async (req, res) => {
  try {
    ...
    user
      ...
      : res
          .status(404)
          .json({ message: `Couldn't find user: ${req.params.id}` });
  } catch (err) {
...
router.get("/:username/logs", async (req, res) => {
  try {
    ...
    logs.length > 0
      ...
      : res
          .status(404)
          .json({ message: `Couldn't find logs for ${req.params.username}` });
  } catch (err) {
...
router.get("/:username/logs/:exercise", async (req, res) => {
  try {
    ...
    logs.length > 0
      ? res.status(200).json(logs)
      : res.status(404).json({
          message: `Couldn't find a log for exercise ${
            req.params.exercise
...
router.get("/:username/goals", async (req, res) => {
  try {
    ...
    goals.length > 0
      ...
      : res
          .status(404)
          .json({ message: `Couldn't find goals for ${req.params.username}` });
  } catch (err) {
...
router.get("/:username/goals/:exercise", async (req, res) => {
  try {
    ...
    goals.length > 0
      ? res.status(200).json(goals)
      : res.status(404).json({
          message: `Couldn't find a goal for exercise ${
            req.params.exercise
          } for user ${req.params.username}.`
        });
  } catch (err) {
